...
////  ProjectICG
////
////  Created by Nishita Kharche on 4/22/23.
////
//
//#include <stdio.h>
//
...
//Vec3f lightDirectionVec(0.0,1.0,40.0);
...
//
//
//Vec3f vertPosArr[17481]; //dragon - 7759
//int finalVertices[17481];
//struct Tetrahedron tetrahedrons[65496]; //dragon - 32959
//struct Face faces[26922]; //11640 dragon - 5921
//Vec3f normArray[17482] = {Vec3f(0.0)}; //dragon - 5921
//
//size_t vertPosArraySize = 0;
//size_t faceArraySize = 0;
//size_t normArraySize = 0;
//
//static void createVertexBufferTet(Vec3f vertPosArray[], size_t vertPosArraySize, Vec3f normArray[], size_t normArraySize, struct Face face[], size_t faceArraySize){
//    
//    glGenBuffers(1, &vbo0);
//    glBindBuffer(GL_ARRAY_BUFFER, vbo0);
//    glBufferData(GL_ARRAY_BUFFER, sizeof(Vec3f) * vertPosArraySize, vertPosArray, GL_STATIC_DRAW);
//    
//    glGenBuffers(1, &vbo1);
//    glBindBuffer(GL_ARRAY_BUFFER, vbo1);
//    glBufferData(GL_ARRAY_BUFFER, sizeof(Vec3f) * vertPosArraySize, normArray, GL_STATIC_DRAW);
//    
...
//
//struct Particle {
//    float mass;// For simplicity, we'll assume tetrahedral elements
//    Vec3f pos;
//    Vec3f vel;
//};
//
...
//    ifstream nodeFile((objectFileRoot + "armadillo.node").c_str());
//    if (nodeFile.is_open()) {
//      string line;
...
//    ifstream eleFile((objectFileRoot + "armadillo.ele").c_str());
//    if (eleFile.is_open()) {
//      string line;
...
//
//    int index = 0;
//    map<array<int, 3>,set<int>> face_map;
//    for (Tetrahedron& t : tetrahedrons) {
////        array<int, 3> face1 = {t.v[0], t.v[1], t.v[2]};
////        array<int, 3> face2 = {t.v[1], t.v[2], t.v[3]};
////        array<int, 3> face3 = {t.v[2], t.v[3], t.v[0]};
////        array<int, 3> face4 = {t.v[3], t.v[0], t.v[1]};
////        sort(face1.begin(), face1.end());
////        face_map[face1].insert(t.id);
////        sort(face2.begin(), face2.end());
////        face_map[face2].insert(t.id);
////        sort(face3.begin(), face3.end());
////        face_map[face3].insert(t.id);
////        sort(face4.begin(), face4.end());
////        face_map[face4].insert(t.id);
//        
//        
//        array<int, 3> face1 = {t.v[0], t.v[1], t.v[2]};
//        array<int, 3> face2 = {t.v[1], t.v[2], t.v[3]};
...
//            face.v[2] = key[2];
//            faces[index] = face;
//            cy::Vec3f v1 = vertPosArr[face.v[0]];
//            cy::Vec3f v2 = vertPosArr[face.v[1]];
//            cy::Vec3f v3 = vertPosArr[face.v[2]];
//            
//            
////            tetrahedrons[value[0]];
//            cy::Vec3f v4;
//            for (Tetrahedron& t : tetrahedrons) {
//                std::set<int>::iterator it=value.begin();
//                if(t.id == *it){
//                    for (int i = 0; i < 4; i++) {
...
//            }
//                
//            cy::Vec3f norm = (v2-v1).Cross(v3-v1);
//            float dotProduct = norm.Dot(v4-v1);
//            if(dotProduct >= 0.0){
//                norm = -norm;
//            }
//            norm.Normalize();
//            normArray[face.v[0]] = norm;
//            normArray[face.v[1]] = norm;
//            normArray[face.v[2]] = norm;
//            
////            cout<<"face "<< index <<" : "<<face.v[0]<<" "<<face.v[1]<<" "<<face.v[2]<<endl;
////            cout<<"normArray "<< index <<" : "<<norm.x<<" "<<norm.y<<" "<<norm.z<<endl;
//            index++;
//        }
//    }
//    
////    for(int i=0; i< sizeof(vertexPosArray)/sizeof(vertexPosArray[0]); i++){
//////        vertexPosArray[i] = vertices[i];
////        cout<<"vertexPosArray "<< i <<" : "<<vertexPosArray[i].x<<" "<<vertexPosArray[i].y<<" "<<vertexPosArray[i].z<<endl;
////    }
//    
//    vertPosArraySize = sizeof(vertPosArr)/sizeof(vertPosArr[0]);
//    faceArraySize = sizeof(faces)/sizeof(faces[0]);
//    normArraySize = sizeof(normArray)/sizeof(normArray[0]);
//    
//    
//    
//    
...
//    createVertexArray();
//    //change
//    createVertexBufferTet(vertPosArr, vertPosArraySize, normArray, normArraySize, faces, faceArraySize);
...
//        glUseProgram(progTeapot.GetID());
//        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebuffer);
//        glDrawElements(GL_TRIANGLES, sizeof(faces[0]) * faceArraySize, GL_UNSIGNED_INT, 0); // CHANGE THIS
