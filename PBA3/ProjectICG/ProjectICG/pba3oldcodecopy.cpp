...
//};
//
//struct Tetrahedron tetrahedrons[1]; //dragon - 32959, armadillo - 65496, cube 5
//struct Face faces[4]; //11640 dragon - 5921, armadillo - 26922 // has all the surface faces only, cube 12
//
//Vec3f vertPosArr[4]; //dragon - 7759, armadillo - 17481, cube 8 // update at each iteration TODO remove translation matrix
//Vec3f normArr[4] = {Vec3f(0.0)}; //dragon - 5921, armadillo - 17482, cube 8
//
...
//
//
//float kJacobian[12][12] = {0.0f};
//float massM[12][12] = {0.0f};
//float b[12][1] = {0.0f};
//float a[12][12] = {0.0f};
//
//struct Particle {
...
//                           0.0, 0.0, 0.0);
//};
//struct Particle particles[4];// 8 cube
//int particleSize = 4;//8 cube
//
...
//    }
//}
//struct Edge edges[6];//18 cube
//Vec3f gravity = Vec3f(0.0f, -9.8, 0.0f);
//float dt3 = 0.0005;
//float floorHeight = -20.0;
//
...
//                    kJacobian[p0+col][p0+row] = dFtotal(row, col);
//                    kJacobian[p0+col][p1+row] = -dFtotal(row, col);
//                    kJacobian[p1+col][p0+row] = -dFtotal(row, col);
//                    kJacobian[p1+col][p1+row] = dFtotal(row, col);
//                }
//            }
//            
//        }
...
//        
//        b[i*3][0] = p.vel.x + dt3 * p.force.x;
//        b[i*3+1][0] = p.vel.y + dt3 * p.force.y;
//        b[i*3+2][0] = p.vel.z + dt3 * p.force.z;
//        
//    }
//    
//    // v t+1 is 3x1 vector
...
//        particles[idx].vel = -particles[idx].vel;
//    }
//}
//
...
//    for (int i=0; i<vertPosArrSize; i++){
//        particles[i].force = particles[i].force + gravity;
//        particles[i].vel = particles[i].vel + dt3*particles[i].force;
...
//        
//        // check if touches surface - then make vel -ve
//        bounceFromFloor(i);
//    }
//
//}
...
//    for(int i=0; i<(3*vertPosArrSize); i++){
//        massM[i][i] = 1.0;
//    }
//}
//
